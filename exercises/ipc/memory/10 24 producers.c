...
void a_producer(char* program) {
    ...
    for(k = 0; k < 10; k++){ //lo voy a hacer 10 veces, no tiene que ver con SIZE
        ...
        sem_wait(semid, EMPTY, i); /* ver si hay espacios para crear esos i items. Si no 
                                    los hay, espera */
        ...
        printf("Producer %i is putting the following products:\n", getpid());
        for ( j = 0; j < i; j++ ){
        ...
        sem_signal(semid, FULL, i); /* El companion de sem_wait(semid, EMPTY, i) */
